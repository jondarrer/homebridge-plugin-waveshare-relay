...
export class WaveshareRelayHomebridgePlatform implements IDynamicPlatformPlugin {
  ...
  public waveshareRelayApi: WaveshareRelayApi;
  ...
  discoverDevices = async () => {
    ...
    for (const relay of relays) {
      const { id: relayId } = relay;
      this.log.debug(`Processing relay ${relayId}.`);
    ...
    for (let i = 0; i < this.accessories.length; i++) {
      ...
      if (!relays.find(relay => relay.id === (existingAccessory.context as IWaveshareRelay).id)) {
